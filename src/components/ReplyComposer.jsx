...
// onSend({ html, text, files, sendAt })  — sendAt null = send now, else ISO string.
export default function ReplyComposer({ onSend, sending, channel, kb = [], initialHtml = '' }) {
  const ed = useRef(null)
  const [files, setFiles] = useState([])
  const [schedOpen, setSchedOpen] = useState(false)
  const [customAt, setCustomAt] = useState('')

  const exec = (cmd, val) => { document.execCommand(cmd, false, val); ed.current?.focus() }
  ...
  const collect = () => ({ html: ed.current?.innerHTML || '', text: (ed.current?.innerText || '').trim() })
  const doSend = (sendAt) => {
    const { html, text } = collect()
    if (!text && files.length === 0) return
    onSend({ html, text, files, sendAt })
    if (!sendAt || sendAt) { ed.current.innerHTML = ''; setFiles([]); setSchedOpen(false) }
  }

  // schedule presets (computed in the browser)
  const presets = () => {
    const now = new Date()
    ...
    return [
      ...
      { label: 'יום ראשון 09:00', at: sunday },
    ]
  }
  ...
  return (
    <div className="composer">
      ...
      <div
        ref={ed} className="composer-editor" contentEditable suppressContentEditableWarning
      ...
      <div className="composer-foot">
        ...
        <div style={{ position: 'relative' }}>
          ...
          {schedOpen && (
            <div className="pop" style={{ position: 'absolute', bottom: 44, insetInlineEnd: 0, width: 230, padding: 10, zIndex: 40 }}>
              {presets().map(p => (
                <button key={p.label} className="btn subtle sm block" style={{ marginBottom: 6, justifyContent: 'space-between' }}
                  onClick={() => doSend(p.at.toISOString())}>{p.label} <span className="small muted">{p.at.toLocaleString('he-IL', { day: '2-digit', month: '2-digit', hour: '2-digit', minute: '2-digit' })}</span></button>
              ))}
              ...
              <button className="btn sm block" disabled={!customAt} onClick={() => doSend(new Date(customAt).toISOString())}>תזמן שליחה</button>
            </div>
          )}
        </div>
