...
export default function RepDetail() {
  ...
  const related = [
    { key: 'students', label: 'תלמידים / לידים', count: students.length, rows: students, onOpen: r => `/people/${r.id}`, columns: [{ label: 'שם', get: r => r.full_name }, { label: 'טלפון', get: r => <span dir="ltr">{r.phone || '-'}</span> }, { label: 'סטטוס', get: r => <span className={`badge ${SALES_STATUS_META[r.sales_status]?.badge || 'gray'}`}>{SALES_STATUS_META[r.sales_status]?.label || '-'}</span> }] },
    { key: 'opps', label: 'הזדמנויות', count: opps.length, rows: opps, onOpen: r => `/opportunities/${r.id}`, columns: [{ label: 'לקוח', get: r => r.person?.full_name || '-' }, { label: 'סוג', get: r => r.training_type || '-' }, { label: 'סטטוס', get: r => <span className={`badge ${OPP_STATUS[r.status]?.badge || 'gray'}`}>{OPP_STATUS[r.status]?.label || '-'}</span> }] },
