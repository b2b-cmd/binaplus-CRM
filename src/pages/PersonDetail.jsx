  ...
  const related = [
    { key: 'orders', label: 'הזמנות', count: orders.length, rows: orders, onOpen: r => `/orders/${r.id}`,
      ...
      listColumns: [
        ...
        { source: 'deposit', label: 'מקדמה', hidden: true, render: r => r.deposit ? `₪${r.deposit.toLocaleString()}` : '-' },
        { source: 'remaining', label: 'נותר', hidden: true, render: r => r.remaining ? `₪${r.remaining.toLocaleString()}` : '-' },
        { source: 'close_date', label: 'תאריך סגירה', hidden: true, render: r => r.close_date ? new Date(r.close_date).toLocaleDateString('he-IL') : '-' },
        { source: 'agreement_status', label: 'הסכם', hidden: true, render: r => r.agreement_status || '-' },
      ],
    ...
    { key: 'tk', label: 'פניות', count: tickets.length, rows: tickets, onOpen: r => `/tickets/${r.id}`,
      ...
      listColumns: [
        ...
        { source: 'urgency', label: 'דחיפות', hidden: true, render: r => r.urgency || '-' },
        { source: 'channel', label: 'ערוץ', hidden: true, render: r => r.channel || '-' },
        { source: 'handled_by', label: 'טופל ע"י', hidden: true, render: r => r.handled_by || '-' },
      ],
